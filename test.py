...
SUMMARIZE_PROMPT = """请将以下搜索结果压缩为一份结构化摘要，保留关键事实和引用编号。要求：
- 按主题归类，去掉重复信息
- 保留每条信息的引用编号（如 [1]）以便最终答案引用
- 只保留与问题相关的内容，无关信息直接丢弃
...
ANSWER_PROMPT = """你是专业研究助理。请根据以下摘要，回答用户问题。要求：
1. 内容准确、结构清晰
2. 必须为每个事实标注引用编号（如 [1]）
...
def generate_answer(state: ResearchState) -> dict:
    ...
    # 第一步：用 flash 压缩搜索结果，减少输入 token
    t1 = time.perf_counter()
    print(f"  第一步: flash 压缩搜索结果...")
    summary_prompt = SUMMARIZE_PROMPT.format(question=state["question"], context=context)
    summary = llm_fast.invoke(summary_prompt).content
    print(f"    原始: {len(context)} 字 → 摘要: {len(summary)} 字 ({len(summary)*100//len(context)}%)")
    print(f"    耗时: {time.perf_counter() - t1:.1f}s")

    # 第二步：用 pro 基于摘要生成答案
    t2 = time.perf_counter()
    print(f"  第二步: pro 生成最终答案...")
    answer_prompt = ANSWER_PROMPT.format(question=state["question"], context=summary)
    response = llm.invoke(answer_prompt)
    print(f"    耗时: {time.perf_counter() - t2:.1f}s")
